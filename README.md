...
它以内置 V4 作为稳定视觉基因：暗部渐变、中文信息层级、蓝绿红黄语义色、细描边深色卡片、柔和帧动画，以及严格跟随字幕且不交叠的时间轴。V4 不是封闭模板，Skill 会根据台词关系选择、变通或创造新的 Remotion 表现方式。
...
- 内置整词级联、状态替换、阶段轨道、一次性确认、真实数字增长和数据条等可选能力，但不会机械调用。
